...
def build_glass_nav_bar(navigation_items, selected_key, on_select):
    def _on_hover(e, is_selected):
        if not is_selected:
            e.control.bgcolor = (
                ft.Colors.with_opacity(0.12, "#D9E5ED")
                if e.data == "true"
    ...
    for key, label, icon_name in navigation_items:
        ...
        items.append(
            ft.Container(
                ...
                bgcolor=ft.Colors.with_opacity(0.28, PRIMARY) if is_selected else ft.Colors.TRANSPARENT,
                border=ft.border.all(
                    1,
                    ft.Colors.with_opacity(0.48, PRIMARY) if is_selected else ft.Colors.TRANSPARENT,
                ),
                ...
                content=ft.Column(
                    ...
                    controls=[
                        ft.Icon(
                            icon,
                            color=ft.Colors.WHITE if is_selected else "#AEBFCA",
                            size=18,
                        ),
                        ft.Text(
                            label,
    ...
    return ft.Container(
        ...
        height=78,
        border_radius=28,
        bgcolor=ft.Colors.with_opacity(0.72, "#182833"),
