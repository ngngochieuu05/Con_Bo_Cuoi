...
def build_app_taskbar(ctk, parent, *, title: str, subtitle: str = "", status_text: str | None = None,
    ...
    if logo_img:
        logo_lbl = ctk.CTkLabel(inner, image=logo_img, text="")
...
def build_page_body(ctk, parent, *, padx: int = 10, pady: int = 10):
    shell = ctk.CTkFrame(parent, fg_color=BG, corner_radius=18, border_width=0)
...
def build_card(ctk, parent, *, elevated: bool = False, fg_color: str = BG2):
    return ctk.CTkFrame(
        parent,
        fg_color=fg_color,
        corner_radius=CARD_RADIUS if not elevated else CARD_RADIUS + 2,
        border_width=1,
        border_color=BORDER,
    )
