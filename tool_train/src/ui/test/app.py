...
class TesterApp(ctk.CTk):
    ...
    def _make_tab_shell(self, parent):
        return build_page_body(ctk, parent, padx=6, pady=8)
    ...
    def _cmp_draw_slot_chart(self, slot: int):
        ...
        if history:
            ...
            series_specs = [
                ("train_loss", "#F59E0B", False, "Train Loss"),
                ("val_loss", "#EF4444", False, "Val Loss"),
    ...
    def _bench_build_reference_table(self, your_model: dict | None = None):
        ...
        rows_data = [
            ...
            ("Val Loss", "—", _fmt_num(reimpl_model.get("val_loss"), 4), _fmt_num(yolo_model.get("val_loss"), 4)),
    ...
    def _bench_save_chart(self):
        """Lưu biểu đồ so sánh bài báo ra file."""
        from tkinter import filedialog as _fd
        import datetime
        if not self._bench_tk_img:
            messagebox.showinfo("Chưa có chart", "Chạy Đánh giá hoặc Batch Predict trước.")
            return
        ts = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        path = _fd.asksaveasfilename(
            title="Lưu biểu đồ so sánh bài báo",
            defaultextension=".png",
            initialfile=f"benchmark_comparison_{ts}",
        ...
        try:
            ...
            import matplotlib.pyplot as plt
            import matplotlib.gridspec as gridspec
            # Re-use last bench_tk_img workaround: just grab canvas as PostScript
            # Simple: save the existing PhotoImage
            self._bench_tk_img.__class__  # just a check
            # Export via PIL
            from PIL import ImageTk as _ITK, Image as _PIL
            # We can't directly convert PhotoImage back; re-draw to file via the canvas widget
            self._bench_canvas.postscript(file=path.replace(".png", ".ps"),
                                          colormode="color")
            messagebox.showinfo("✔ Đã lưu", f"PostScript: {path.replace('.png', '.ps')}")
        except Exception as ex:
    ...
    def _build_batch_tab(self, parent: tk.Frame):
        ...
        tk.Label(log_hdr, text="📝 Log Batch Predict",
        ...
        self._batch_chart_canvas.create_text(
            200, 100, text="📊 Chạy Batch Predict để xem biểu đồ",
    ...
    def _batch_save_chart(self):
        ...
        save_path = _fd.asksaveasfilename(
            title="Lưu biểu đồ Batch Predict",
            defaultextension=".png",
        ...
        if not save_path:
            return
        c = self._batch_chart_canvas
        # Nếu có ảnh đã vẽ dùng PIL, save lại từ tk_img
        tk_img = getattr(self, "_batch_chart_tk_img", None)
        if tk_img is None:
            messagebox.showinfo("ℹ", "Chưa có biểu đồ để lưu. Hãy chạy Batch Predict trước.")
            return
        try:
            from PIL import ImageTk as _ITK, Image as _PIL
            pil_img = _ITK.getimage(tk_img)
            pil_img.save(save_path)
            messagebox.showinfo("✔ Đã lưu", f"Biểu đồ đã lưu:\n{save_path}")
        except Exception as ex:
    ...
    def _eval_draw_confusion_matrix(self, class_names: list, matrix: list, title: str = "Confusion Matrix"):
        ...
        try:
            ...
            ax.set_xlabel("Predicted", color="#c0c0e0", fontsize=8)
            ax.set_ylabel("Ground Truth", color="#c0c0e0", fontsize=8)
...
if HAS_SK:
    try:
        ...
        ax.set_ylabel("Ground Truth", color="white", fontsize=11)
        ...
        for i in range(num_classes):
            for j in range(num_classes):
                ax.text(j,i,str(cm[i,j]),ha="center",va="center",fontsize=11,fontweight="bold",
                        color="white" if cm[i,j]<thresh else "#1e1e2e")
        ax.set_xlabel("Predicted",color="white",fontsize=11); ax.set_ylabel("Ground Truth",color="white",fontsize=11)
...
"""
            ...
            try:
                ...
                if mode == "summary":
                    ...
                    if summary_labels:
                        ...
                        ax.set_title("Validation Metrics", color="#c0c0e0", fontsize=12, pad=8)
                        top_max = max(summary_values) if summary_values else 0.0
                        for idx, bar in enumerate(bars):
                            val = summary_values[idx]
                            txt = f"{val:.4f}" if summary_labels[idx] == "Val Loss" else f"{val:.2f}%"
                            ax.text(
                    ...
                    ax.plot(
                        ...
                        color="#f97316", linewidth=1.8, marker=".", markersize=4, label="Train Loss"
                    )
                    if any(v is not None for v in val_loss_hist):
                        ax.plot(
                            epochs,
                            [v if v is not None else float("nan") for v in val_loss_hist],
                            color="#ef4444", linewidth=1.8, marker=".", markersize=4, label="Val Loss"
                        )
                    ax2 = ax.twinx()
                    ...
                    ax.set_xlabel("Predicted", color="#9090b0", fontsize=8)
                    ax.set_ylabel("Ground Truth", color="#9090b0", fontsize=8)
    ...
    def _paper_save_chart(self, which: str):
        """Lưu biểu đồ bar chart JILSA / PLOS thành file PNG — clone từ _batch_save_chart."""
        from tkinter import filedialog as _fd
        import datetime

        tk_img = (self._jilsa_chart_tk_img if which == "jilsa"
                  else self._plos_chart_tk_img)
        if tk_img is None:
            messagebox.showinfo("ℹ", "Chưa có biểu đồ để lưu.\nHãy chạy đánh giá trước.")
            return

        paper_label = "JILSA2022" if which == "jilsa" else "PLOSONE2024"
        default_name = f"chart_{paper_label}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
        save_path = _fd.asksaveasfilename(
    ...
    def _hist_render_cm(self, canvas: tk.Canvas, d: dict, accent: str):
        ...
        try:
            ...
            ax.set_ylabel("Ground Truth", color="white", fontsize=10)
